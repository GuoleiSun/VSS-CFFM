...
class LayerNorm(nn.Module):
    def __init__(self, dim, eps=1e-05):
        ...
        self.eps = eps
        self.dim = dim
        self.weight = nn.parameter.Parameter(torch.ones(dim, dtype=torch.float32))
        self.bias = nn.parameter.Parameter(torch.zeros(dim, dtype=torch.float32))

    def forward(self, x):
        mean = torch.mean(x, 1, keepdim=True)
        var = torch.var(x, 1, False, keepdim=True)
        x = (x - mean) / torch.sqrt(var + self.eps) * self.weight.reshape(self.dim, 1, 1) + self.bias.reshape(self.dim, 1, 1)
        return x
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sr_ratio > 1:
            ...
            kv = self.kv(x_).reshape(B, 2, self.num_heads, C // self.num_heads, -1).transpose(0, 1)
        else:
            kv = self.kv(x).reshape(B, 2, self.num_heads, C // self.num_heads, -1).transpose(0, 1)
        k, v = kv[0], kv[1]
        ...
        x = (attn @ v.transpose(-2, -1)).transpose(-2, -1).reshape(B, C, H, W)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class mit_b0_fast(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(mit_b0_fast, self).__init__(
...
@BACKBONES.register_module()
class mit_b1_fast(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(mit_b1_fast, self).__init__(
...
@BACKBONES.register_module()
class mit_b2_fast(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(mit_b2_fast, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
...
@BACKBONES.register_module()
class mit_b3_fast(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(mit_b3_fast, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
...
@BACKBONES.register_module()
class mit_b4_fast(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(mit_b4_fast, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
...
@BACKBONES.register_module()
class mit_b5_fast(MixVisionTransformer):
    def __init__(self, **kwargs):
        super(mit_b5_fast, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
