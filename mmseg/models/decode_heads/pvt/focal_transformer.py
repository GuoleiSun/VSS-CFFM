...
class WindowAttention3d(nn.Module):
    ...
    def __init__(self, dim, expand_size, window_size, focal_window, focal_level, num_heads, 
        ...
        if self.expand_size > 0 and focal_level > 0:
            ...
            self.window_size_of_key = self.window_size[0] * self.window_size[1] if self.expand_size == 0 else \
                (4 * self.window_size[0] * self.window_size[1] - 4 * (self.window_size[0] -  self.expand_size) * (self.window_size[0] -  self.expand_size))        
            self.relative_position_bias_table_to_neighbors = nn.Parameter(
                torch.zeros(1, num_heads, self.window_size[0] * self.window_size[1], num_clips*self.window_size_of_key))  # Wh*Ww, nH, nSurrounding
            trunc_normal_(self.relative_position_bias_table_to_neighbors, std=.02)
        ...
        if pool_method != "none" and focal_level > 1:
            ...
            for k in range(focal_level-1):
                ...
                relative_position_bias_table_to_windows = nn.Parameter(
                    torch.zeros(
                        self.num_heads,
                        (2*num_clips-1)*(self.window_size[0] + self.focal_window + 2**k - 2) * (self.window_size[1] + self.focal_window + 2**k - 2), 
                        )
                )
                trunc_normal_(relative_position_bias_table_to_windows, std=.02)
                self.relative_position_bias_table_to_windows.append(relative_position_bias_table_to_windows)
    ...
    def forward(self, x_all, mask_all=None, batch_size=None, num_clips=None):
        ...
        window_area_rolled = num_clips*k_rolled.shape[2]
        ...
        # add relative position bias for patches inside a window
        if self.expand_size > 0 and self.focal_level > 0:
            attn[:, :, :window_area, window_area_clips:window_area_rolled] = attn[:, :, :window_area, window_area_clips:window_area_rolled] + self.relative_position_bias_table_to_neighbors
...
class WindowAttention3d2(nn.Module):
    ...
    def __init__(self, dim, expand_size, window_size, focal_window, focal_level, num_heads, 
                    qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., pool_method="none", focal_l_clips=[16,8,2], focal_kernel_clips=[7,5,3]):
        ...
        if pool_method != "none" and focal_level > 1:
            ...
            for k in range(len(focal_l_clips)):
                ...
                if 2**focal_l_clips[k]>self.window_size[0]:
                    stride=1
                    padding=0
                    kernel_size=focal_kernel_clips[k]
    ...
    def forward(self, x_all, mask_all=None, batch_size=None, num_clips=None):
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(len(self.focal_l_clips)):
                focal_l_big_flag=False
                if 2**self.focal_l_clips[k]>self.window_size[0]:
                    stride=1
                    focal_l_big_flag=True
                else:
                    stride = 2**self.focal_l_clips[k]
...
class WindowAttention3d3_selfatten(nn.Module):
    r""" Window based multi-head self attention (W-MSA) module with relative position bias.
    ...
    def forward(self, x_all, mask_all=None, batch_size=None, num_clips=None):
        ...
        ## contexts from the target frame (pixel-wise fine-grained tokens)
        if self.expand_size > 0 and self.focal_level > 0:
            (k_tl, v_tl) = map(
                lambda t: torch.roll(t, shifts=(-self.expand_size, -self.expand_size), dims=(1, 2)), (k, v)
            )
        ...
        ## contexts from the target frame (coarse tokens) and the reference frame (fine-to-coarse depending on the distance from the target)
        if self.pool_method != "none" and self.focal_level > 1:
            k_pooled = []
            v_pooled = []

            ## contexts from the target frame (coarse tokens)
            for k in range(self.focal_level-1):
                stride = 2**k
                x_window_pooled = x_all[0][k+1]  # B0, nWh, nWw, C
                nWh, nWw = x_window_pooled.shape[1:3] 
            ...
            ## contexts from the reference frames
            for k in range(len(self.focal_l_clips)):
                focal_l_big_flag=False
                if self.focal_l_clips[k]>self.window_size[0]:
                    stride=1
        ...
        # exit()
        attn = (q_windows @ k_all.transpose(-2, -1))  # B0*nW, nHead, window_size*window_size, focal_window_size*focal_window_size
        attn22 = k_all @ k_all.transpose(-2, -1)  # added to test the speed
        ...
        x = (attn @ v_all).transpose(1, 2).reshape(attn.shape[0], window_area, C)
        x22=(attn22 @ v_all).transpose(1, 2).reshape(attn.shape[0], -1, C)
        x = self.proj(x)
        x = self.proj_drop(x)

        x22=self.proj(x22)
        x22=self.proj_drop(x22)
        # print(x.shape)
        # x = x.view(B/num_clips, nH, nW, C )
        # exit()
        return x
...
class WindowAttention3d3_onlycffm(nn.Module):
    r""" Window based multi-head self attention (W-MSA) module with relative position bias.
    ...
    def forward(self, x_all, mask_all=None, batch_size=None, num_clips=None):
        ...
        # x = (attn @ v_all).transpose(1, 2).reshape(attn.shape[0], window_area, C)
        x = q_windows + k_all.mean(-2, keepdim=True)
        x=x.transpose(1, 2).reshape(q_windows.shape[0], window_area, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        # print(x.shape)
        # x = x.view(B/num_clips, nH, nW, C )
...
#         BD, nH, nW, C = x.shape
#         qkv = self.qkv(x).reshape(BD, nH, nW, 3, C).permute(3, 0, 1, 2, 4).contiguous()
#         q, k, v = qkv[0], qkv[1], qkv[2]  # BD, nH, nW, C
...
class FocalTransformerBlock3d(nn.Module):
    r""" Focal Transformer Block.
    ...
    def forward(self, x):
        ...
        attn_windows = self.attn(x_windows_all, mask_all=x_window_masks_all, batch_size=B0, num_clips=D0)  # nW*B0, window_size*window_size, C
...
class FocalTransformerBlock3d2(nn.Module):
    r""" Focal Transformer Block.
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, pool_method="none", 
                 focal_level=1, focal_window=1, use_layerscale=False, layerscale_value=1e-4, focal_l_clips=[14,0,1], focal_kernel_clips=[7,5,3]):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
        ...
        if self.pool_method != "none":
            ...
            for k in range(len(focal_l_clips)):
                # window_size_glo = math.floor(self.window_size_glo / (2 ** k))
                if 2**focal_l_clips[k]>self.window_size:
                    window_size_glo = focal_l_clips[k]
                else:
                    window_size_glo = math.floor(self.window_size_glo / (2 ** focal_l_clips[k]))
                # window_size_glo = focal_l_clips[k]
                if self.pool_method == "fc":
    ...
    def forward(self, x):
        ...
        if self.focal_level > 1 and self.pool_method != "none": 
            ...
            for k in range(len(self.focal_l_clips)):     
                ...
                # pooled_w = math.ceil(W / self.window_size) * (2 ** k)
                # window_size_glo=focal_l_clips[k]
                if 2**self.focal_l_clips[k]>self.window_size:
                    window_size_glo = self.focal_l_clips[k]
                else:
                    window_size_glo = math.floor(self.window_size_glo / (2 ** self.focal_l_clips[k]))
                # pooled_h = math.ceil(H / window_size_glo)
                # pooled_w = math.ceil(W / window_size_glo)
                    pooled_h = math.ceil(H / self.window_size) * (2 ** self.focal_l_clips[k])
                    pooled_w = math.ceil(W / self.window_size) * (2 ** self.focal_l_clips[k])
...
class FocalTransformerBlock3d3(nn.Module):
    r""" Focal Transformer Block.
...
class FocalTransformerBlock3d3_selfatten(nn.Module):
    r""" Focal Transformer Block.
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        self.attn = WindowAttention3d3_selfatten(
            dim, expand_size=self.expand_size, window_size=to_2tuple(self.window_size), 
            focal_window=focal_window, focal_level=focal_level, num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, pool_method=pool_method, focal_l_clips=focal_l_clips, focal_kernel_clips=focal_kernel_clips)
...
class FocalTransformerBlock3d3_onlycffm(nn.Module):
    r""" Focal Transformer Block.
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        self.attn = WindowAttention3d3_onlycffm(
            dim, expand_size=self.expand_size, window_size=to_2tuple(self.window_size), 
            focal_window=focal_window, focal_level=focal_level, num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, pool_method=pool_method, focal_l_clips=focal_l_clips, focal_kernel_clips=focal_kernel_clips)
...
class FocalTransformerBlock3d3_nochange(nn.Module):
    r""" Focal Transformer Block.

    Theoritically, this class is the same as FocalTransformerBlock3d3
    Args:
        dim (int): Number of input channels.
        input_resolution (tuple[int]): Input resulotion.
        num_heads (int): Number of attention heads.
    ...
    def forward(self, x, layer_id=0):
        ...
        if layer_id==0:
            ...
            if self.focal_level > 1 and self.pool_method != "none": 
                ...
                for k in range(len(self.focal_l_clips)):     
                    ...
                    x_window_masks_all_clips += [None]
                    # pooling_index=pooling_index+1
                    x_references.append(x_windows_noreshape)
        else:
            assert type(x)==list
            B0, H0, W0, C = x[-1].shape
            x_target = self.norm1(x[-1])
            D0=len(x)
            pad_l = pad_t = 0
            pad_r = (self.window_size - W0 % self.window_size) % self.window_size
            pad_b = (self.window_size - H0 % self.window_size) % self.window_size
            if pad_r > 0 or pad_b > 0:
                x_target = F.pad(x_target, (0, 0, pad_l, pad_r, pad_t, pad_b))
            B0, H, W, C = x_target.shape
            if self.shift_size > 0:
                shifted_x = torch.roll(x_target, shifts=(-self.shift_size, -self.shift_size), dims=(2, 3))
            else:
                shifted_x = x_target
            x_windows_all = [shifted_x]
            x_windows_all_clips=[]
        ...
        # reverse cyclic shift
        if self.shift_size > 0:
            x_target = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        else:
            x_target = shifted_x
        # x = x[:, :self.input_resolution[0], :self.input_resolution[1]].contiguous().view(B, -1, C)
        x_target = x_target[:, :H0, :W0].contiguous().view(B0, -1, C)
        ...
        # assert x.shape==shortcut.shape
        if layer_id==0:
            x_target = shortcut[:,-1].view(B0, -1, C) + self.drop_path(x_target if (not self.use_layerscale) else (self.gamma_1 * x_target))
            x_target = x_target + self.drop_path(self.mlp(self.norm2(x_target)) if (not self.use_layerscale) else (self.gamma_2 * self.mlp(self.norm2(x_target))))
            x=x_references+[x_target.view(B0,H0,W0,C)]
        else:
            x_target = x[-1].view(B0, -1, C) + self.drop_path(x_target if (not self.use_layerscale) else (self.gamma_1 * x_target))
            x_target = x_target + self.drop_path(self.mlp(self.norm2(x_target)) if (not self.use_layerscale) else (self.gamma_2 * self.mlp(self.norm2(x_target))))
            x[-1]=x_target.view(B0,H0,W0,C)
...
#         # shifted_x=shifted_x.reshape(B*D)
#         # x_windows_all = [shifted_x]
#         # x_window_masks_all = [attn_mask]
...
class BasicLayer3d(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer="all",
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            FocalTransformerBlock3d(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, window_size=window_size,
                                 shift_size=(0 if (i % 2 == 0) else window_size // 2) if use_shift else 0,
                                 expand_size=0 if (i % 2 == expand_factor) else expand_size, 
                                 mlp_ratio=mlp_ratio,
...
class BasicLayer3d2(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer="all",
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            FocalTransformerBlock3d2(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, window_size=window_size,
                                 shift_size=(0 if (i % 2 == 0) else window_size // 2) if use_shift else 0,
                                 expand_size=0 if (i % 2 == expand_factor) else expand_size, 
                                 mlp_ratio=mlp_ratio,
...
class BasicLayer3d3(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer="all",
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            FocalTransformerBlock3d3(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, window_size=window_size,
                                 shift_size=(0 if (i % 2 == 0) else window_size // 2) if use_shift else 0,
                                 expand_size=0 if (i % 2 == expand_factor) else expand_size, 
                                 mlp_ratio=mlp_ratio,
...
class BasicLayer3d3_nochange(nn.Module):
    """ A basic Focal Transformer layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer="all",
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            FocalTransformerBlock3d3_nochange(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, window_size=window_size,
                                 shift_size=(0 if (i % 2 == 0) else window_size // 2) if use_shift else 0,
                                 expand_size=0 if (i % 2 == expand_factor) else expand_size, 
                                 mlp_ratio=mlp_ratio,
    ...
    def forward(self, x, batch_size=None, num_clips=None):
        ...
        if self.downsample is not None:
            x = x.view(x.shape[0], self.input_resolution[0], self.input_resolution[1], -1).permute(0, 3, 1, 2).contiguous()
            x = self.downsample(x)
        x = rearrange(x[-1], 'b h w c -> b c h w')
        return x
...
class BasicLayer3d3_selfatten(nn.Module):
    """ A basic Focal Transformer layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer="all",
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            FocalTransformerBlock3d3_selfatten(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, window_size=window_size,
                                 shift_size=(0 if (i % 2 == 0) else window_size // 2) if use_shift else 0,
                                 expand_size=0 if (i % 2 == expand_factor) else expand_size, 
                                 mlp_ratio=mlp_ratio,
...
class BasicLayer3d3_onlycffm(nn.Module):
    """ A basic Focal Transformer layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer="all",
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            FocalTransformerBlock3d3_onlycffm(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, window_size=window_size,
                                 shift_size=(0 if (i % 2 == 0) else window_size // 2) if use_shift else 0,
                                 expand_size=0 if (i % 2 == expand_factor) else expand_size, 
                                 mlp_ratio=mlp_ratio,
